...
//  printf("%d %d %d\n", idx, up_down, ret);
  ...
  if(up_down == -1){
    ret = max(solve(idx-2,1)+board[0][idx], solve(idx-1, 1) + board[0][idx]);
    ret = max(solve(idx-1,-1), ret);
  }
  if(up_down == 1){
    ret = max(solve(idx-2,-1) + board[1][idx], solve(idx-1, -1) + board[1][idx]);
    ret = max(solve(idx-1, 1), ret);
  }
